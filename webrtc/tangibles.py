...
URL = 'https://the-tangibles.appspot.com/room/?r='
...
class PageConnect(webapp.RequestHandler):
  def post(self):
...
class PageDisconnect(webapp.RequestHandler):
  def post(self):
    key = self.request.get('from')
    room_key, user = key.split('/');
...
class PageMessage(webapp.RequestHandler):
  def post(self):
...
class PageRoom(webapp.RequestHandler):
  ...
  def get(self):
    ...
    path = os.path.join(os.path.dirname(__file__), 'room/index.html')
    self.response.out.write(template.render(path, template_values))
    logging.info('User ' + user + ' added to room ' + room_key);
    logging.info('Room ' + room_key + ' has state ' + str(room))


class PageWorkspace(webapp.RequestHandler):
  """The main UI page, renders the 'index.html' template."""
  ...
  def get(self):
    ...
    room_link = 'https://the-tangibles.appspot.com/workspace/?r=' + room_key
    if debug:
      room_link += ('&debug=' + debug)
    if stun_server:
      room_link += ('&ss=' + stun_server)
